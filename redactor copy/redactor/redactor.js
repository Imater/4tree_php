		...
		init: function()
		{
			...
			$(this.doc).keyup($.proxy(function(e)
			{
				var key = e.keyCode || e.which;
					...
					note_saved=false;
					my_autosave = setTimeout( function() { savetext(1); }, 5000 );
...
//////Вставка изображения из буфера
			var who = window.frames['myframe'].document.body;
   ...
   for (var i = 0; i < items.length; ++i) {
       ...
       if (items[i].kind == 'file' && items[i].type.indexOf('image/') !== -1) {
           ...
           return false;
			...
			$(this.doc).keydown($.proxy(function(e)
			{
				var key = e.keyCode || e.which;
					...
					clearTimeout(my_autosave);
					note_saved=false;
					my_autosave = setTimeout( function() { savetext(1); }, 3000 );
		...
		build: function()
		{
			...
			// frame
			this.$frame = $('<iframe frameborder="0" scrolling="auto" style="height: ' + this.height + ';" class="redactor_frame" name="myframe"></iframe>');
		...
		setDoc: function(html)
		{
			...
			'<body><div id="page" class="myfullpage" style="min-height:200px;" contenteditable="true" dir="' + this.opts.direction + '">' +
			html + 
			'</div></body></html>';
		...
		insertHtml: function(html)
		{
					clearTimeout(my_autosave);
					note_saved=false;
					my_autosave = setTimeout( function() { savetext(1); }, 80000 );
		...
		buildButton: function(key, s)
		{
			...
			{
				button.click($.proxy(function(e) { note_saved=false; this[s.func](e); }, this));
			}
		...
		{        
					clearTimeout(my_autosave);
					note_saved=false;
					my_autosave = setTimeout( function() { savetext(1); }, 10000 );
