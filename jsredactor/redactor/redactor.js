...
var rwindow, rdocument,my_autosave,mytesttext,last_editor_click_time;
...
who.onpaste = function (e) {
   ...
   if(e.clipboardData.getData("text/html").indexOf("xml")!=-1) 
...
console.info("need_text",need_text,e.clipboardData.getData("text/html"));

if(!need_text)
{
   
   console.info("clipBoardData = ",e.clipboardData, e.clipboardData.getData("text/html"));
       ...
       if (items[i].kind == 'file' && items[i].type.indexOf('image/') !== -1) {
           ...
           return false;
			...
			this.$editor.keydown($.proxy(function(e)
			{
		       ...
		       if((e.altKey==true) && (e.keyCode==83)) //4tree Alt+s
					{
						e.preventDefault();
						clearTimeout(my_autosave);
						note_saved=false;
...
//			alert(String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'));
			
			mytesttext = str;
			
			str = mytesttext.replace(/(>\n\n)/gm,">");
//			console.info("ss",str);
