...
var Redactor = function(element, options)
{
	...
	this.opts = $.extend({
		...
		allowedTags: ["code", "span", "div", "label", "a", "br", "p", "b", "i", "del", "strike", 
				...
				"th", "tbody", "thead", "tfoot", "h1", "h2", "h3", "h4", "h5", "h6","spoiler"],
...
//			console.info('keyUp',key);
//			if( key == 32) this.insertHtml(' ');
...
who.onpaste = function (e) {
   var items = e.clipboardData.items;
   console.info(e.clipboardData.getData("text/html"));
   for (var i = 0; i < items.length; ++i) {
		console.info(items[i].getAsFile());
       if (items[i].kind == 'file' && items[i].type.indexOf('image/') !== -1) {
           ...
           xhr.open("POST","do.php?save_blob="+$('#redactor_content').attr('notes').replace('node_',''),true);
	...
	paragraphy: function (str)
	{
		...
		// block elements
		var blocks = '(table|thead|tfoot|caption|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|select|form|blockquote|address|math|style|script|object|input|param|p|h[1-6]|spoiler)';
		...
		R('(</?' + blocks + '[^>]*>)\\s*</p>', '$1');
		R('(</?' + blocks + '[^>]*>)\\s*<br />', '$1');
		R('<br />(\\s*</?(p|li|div|dl|dd|dt|th|pre|td|ul|ol|spoiler)[^>]*>)', '$1');
	...
	{
		var etags = ["<pre></pre>","<blockquote>\\s*</blockquote>","<em>\\s*</em>","<ul></ul>","<ol></ol>","<li></li>","<table></table>","<tr></tr>","<span>\\s*<span>", "<span>&nbsp;<span>", "<b>\\s*</b>", "<b>&nbsp;</b>", "<p>\\s*</p>", "<p>&nbsp;</p>",  "<p>\\s*<br>\\s*</p>", "<div>\\s*</div>", "<div>\\s*<br>\\s*</div>", "<spoiler>\\s*</spoiler>"];
	...
	{
		var lb = '\r\n';
		var btags = ["<p", "<form","</ul>", '</ol>', "<fieldset","<legend","<object","<embed","<select","<option","<input","<textarea","<pre","<blockquote","<ul","<ol","<li","<dl","<dt","<dd","<table", "<thead","<tbody","<caption","</caption>","<th","<tr","<td","<figure","<spoiler"];
	...
	{
		var lb = '\r\n';		
		var atags = ['</p>', '</div>', '</h1>', '</h2>', '</h3>', '</h4>', '</h5>', '</h6>', '<br>', '<br />', '</dl>', '</dt>', '</dd>', '</form>', '</blockquote>', '</pre>', '</legend>', '</fieldset>', '</object>', '</embed>', '</textarea>', '</select>', '</option>', '</table>', '</thead>', '</tbody>', '</tr>', '</td>', '</th>', '</figure>','</spoiler>'];
