...
$add = "";
if( stristr("http:/",$image) ) $add = "http://";
else $image = str_replace("http:/", "http://", $image);
...
$minidir = substr($filename_,0,3);


mkdir(CACHE_DIR."/".$minidir, 0755);
define('CACHE_DIR',	CURRENT_DIR . CACHE_DIR_NAME. "/".$minidir);

$dir = "imagecache/".$minidir."/";
...
if (!isset($_GET['nocache']) && file_exists($dir.$filename_for_cache) )
{
//	echo "FROM CACHE".$dir.$filename_for_cache;
	$imageModified	= filemtime($dir.$filename_for_cache);
	
	if(true) {
		$data	= file_get_contents($dir.$filename_for_cache);
	
		$lastModifiedString	= gmdate('D, d M Y H:i:s', $imageModified) . ' GMT';
		$etag				= "CACHE_".md5($data);
		
		$size	= GetImageSize($dir.$filename_for_cache);
		$mime	= $size['mime'];
...
$lfile = fopen($dir . $filename_for_cache, "w");
...
//echo $dir.$filename_for_cache.":".$mime."<hr>";
if (substr($mime, 0, 6) != 'image/')
{
	header('HTTP/1.1 400 Bad Request');
	echo 'Error: requested file is not an accepted type: ' . $dir.$filename_for_cache;
	exit();
}
...
if ((!$maxWidth && !$maxHeight) || (!$color && $maxWidth >= $width && $maxHeight >= $height))
{
	...
	$lastModifiedString	= gmdate('D, d M Y H:i:s', filemtime($dir.$filename_for_cache)) . ' GMT';
	$etag				= md5($data);
	...
	header('Cache-control: max-age='.(60*60*24*365));
	header('Expires: '.gmdate(DATE_RFC1123,time()+60*60*24*365));
//	header('Last-Modified: '.gmdate(DATE_RFC1123,filemtime($image)));
...
$resizedImageSource		.= '-' . $dir.$filename_for_cache;

$resizedImage	= $filename_for_cache;
	
$resized		= $dir . $resizedImage;

// Check the modified times of the cached file and the original file.
// If the original file is older than the cached file, then we simply serve up the cached file
if (!isset($_GET['nocache']) && file_exists($resized) && false)
{
	$imageModified	= filemtime($docRoot . $dir.$filename_for_cache);
...
$src	= $creationFunction($dir.$filename_for_cache);
