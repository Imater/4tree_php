...
if (isset($HTTP_GET_VARS['TEXT'])) 
 {
 if (strlen($HTTP_GET_VARS['TEXT'])<3) exit;
...
if(isset($HTTP_GET_VARS['slava']))
 ...
 if(false) echo 'Закупочная цена складская / Срочная: <b>'.$sql->cost_skl.' / '.$sql->cost_avia.'</b><br>';
 if(false) if ($sql3->amount>0) echo 'Закупочная цена по 1С: <b>'.$sql3->cost1/$sql3->amount.'</b><br><br>';
 if(false) echo 'Рекомендованная ПСР продажная цена (без НДС): <b>'.$sql->cost.'</b><br>';
